...
_SNAPSHOT_DIR = Path.home() / ".claude" / "shell-snapshots"
_TS_RE = re.compile(r"snapshot-bash-(\d+)-")


def _session_time_windows(sessions: list[dict]) -> list[tuple[float, float]]:
    """
    Return (start_ts, end_ts) pairs in Unix seconds for each session.
    Parses first and last JSONL entry timestamps.
    """
    windows = []
    for s in sessions:
        path: Path = s["path"]
        ...
        if first_ts is not None and last_ts is not None:
            # Give a small buffer on each side (5 min) to catch snapshots taken
            # just before or just after the first/last conversation message.
...
class ProjectShellSnapshotsSubTab(QWidget):
    ...
    def _refresh(self):
        ...
        windows = _session_time_windows(sessions)
        if not windows:
            self._list.addItem("Could not determine session time windows.")
            return

        snapshots = sorted(
            _SNAPSHOT_DIR.glob("snapshot-bash-*.sh"),
        ...
        for f, snap_ms in matched:
            ...
            item = QListWidgetItem(f"{dt}  {f.name}")

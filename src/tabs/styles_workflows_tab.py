...
class StylesWorkflowsTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Output styles note (the STANDALONE /output-style command was removed;
        # output styles themselves are still supported).
        deprecation_notice = QLabel(
            "ℹ️ <b>Output styles</b> are still supported (built-in: Default, Proactive, "
            "Concise, Explanatory, Learning; custom: <code>.claude/output-styles/*.md</code>). "
            "The standalone <code>/output-style</code> command was removed in v2.1.91 — "
            "pick a style via <code>/config</code> → Output style, or the <code>outputStyle</code> setting."
        )
    ...
    def load_workflows_info(self, browser):
        ...
        <body style="color: {theme.FG_PRIMARY};">
            ...
            <p>Ask Claude to open a PR, or use the <code>/autofix-pr</code> skill to watch a PR and push fixes.</p>
            ...
            <ul>
                ...
                <li><b>Output styles</b> - reshape the system prompt every turn (<code>/config</code> → Output style)</li>
            </ul>

            <h3 style="color: {theme.ACCENT_SECONDARY}; margin-top: 15px;">🧩 Plugins</h3>
            <p>Extend Claude Code with plugins from a marketplace:</p>
            <pre style="background: {theme.BG_MEDIUM}; padding: 8px; border-radius: 3px;">/plugin marketplace add &lt;org/repo&gt;
claude plugin install &lt;name&gt;@&lt;marketplace&gt;
claude plugin enable &lt;name&gt;</pre>
